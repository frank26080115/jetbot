...
def convert_keras_model_to_tensorrt(inpath):
	...
	model = tf.keras.models.load_model(inpath, compile=False)
	...
	meta = {'input_names': input_names, 'output_names': output_names} # Store additional information in metadata, useful for infrencing
	graph = session.graph
	with graph.as_default():
		...
		with open(output_path, 'wb') as output_file, open(metapath.as_posix(), 'w') as meta_file:
